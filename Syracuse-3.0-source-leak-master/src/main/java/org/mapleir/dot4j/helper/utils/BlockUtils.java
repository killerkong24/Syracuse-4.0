...
public class BlockUtils {
    ...
    public static boolean placeBlock(BlockPos pos, BlockState state) {
        ...
        for (final BlockPos neighbor : neighbors) {
            ...
            final BlockHitResult hit = mc.world.raycastBlock(RotationUtils.getEyesPos(), faceCenter, neighbor, BlockUtils.getBlockState(neighbor).getOutlineShape(mc.world, neighbor), BlockUtils.getBlockState(neighbor));
            if (hit == null) {
        ...
        final ActionResult result = mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, new BlockHitResult(faceCenterToClick, directionToClick, neighborToClick, false));
    ...
    public static boolean rightClickBlock(BlockPos pos) {
        ...
        final VoxelShape shape = state.getOutlineShape(mc.world, pos);
        if (shape.isEmpty())
            return false;
        ...
        final ActionResult result2 = mc.interactionManager.interactBlock(mc.player, Hand.MAIN_HAND, new BlockHitResult(hitVecs[side.ordinal()], side, pos, false));
